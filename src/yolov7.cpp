...
#include "third_party/yolov7/cpp/postprocess.h"
...
Yolov7::Yolov7(std::string model) : IYolo() {
  ...
  ret = init_yolov7_model(model.c_str(), &rknn_app_ctx_);
  if (ret != 0) {
...
DetectResult Yolov7::Detect(cv::Mat img0) {
  ...
  // od_results.count = 0;
  int ret = inference_yolov7_model(&rknn_app_ctx_, &src_image, &od_results);
